...
<%
	...
	StringBuffer valoriCities = new StringBuffer("");
	Map<String, String> cities = DBConnection.getInstance().GetWarrantyDropDownValues("City");

	for (String resp : cities.keySet()) {
		valoriCities.append(resp);
		valoriCities.append(",");
	}

	// RESPONSE + "^" + DISPLAY + "|" + resultSet.getString("TAG");
	
	valoriCities.delete(valoriCities.length() - 1, valoriCities.length()); 
...
<aui:form action="<%=generateWarrantyURL%>" method="POST" name="<portlet:namespace />fm">
	...
	<aui:fieldset-group markupView="lexicon">
			...
			<aui:row style="display: flex;">
				<aui:col>
					<aui:select name="paymentType" label="Payment Type" title="Payment Type" showEmptyOption="true">
						<%
							Map<String, String> payments = DBConnection.getInstance().GetWarrantyDropDownValues("Payment");
							for (String payType : payments.keySet()) {
						%>
						<aui:option label="<%=payments.get(payType)%>" value="<%=payType%>" />
						<%
							}
						%>
					</aui:select>
			...
			<aui:row style="display: flex;">
				<aui:col>
					<aui:select name="end_user_type" showEmptyOption="true" label="End User Type:" title="End User Type">
						<%
							Map<String, String> endUserTypes = DBConnection.getInstance().GetWarrantyDropDownValues("EndUser");
							for (String userType : endUserTypes.keySet()) {
						%>
						<aui:option label="<%=endUserTypes.get(userType)%>" value="<%=userType%>" />
						<%
							}
						%>
					</aui:select>
		...
		<aui:fieldset markupView="lexicon">
			...
			<aui:row style="display: flex;">
				<aui:col>
					<aui:select name="judetSector" label="Judet/Sector" title="Judet/Sector" showEmptyOption="true" required="true"	showRequiredLabel="true" onchange="return LoadCities(this)" >
						<aui:validator name="required" errorMessage="Introduceti Judet Sau Sector!" />
						<%
							Map<String, String> judete = DBConnection.getInstance().GetWarrantyDropDownValues("County");
							for (String judet : judete.keySet()) {
						%>
						<aui:option label="<%=judete.get(judet)%>" value="<%=judet%>" />
						<%
							}
						%>
					</aui:select>
				...
				<aui:col>
					...
					<%-- <aui:select name="orase" label="Orase" title="Orase" showEmptyOption="true" required="true" showRequiredLabel="true"><aui:validator name="required" errorMessage="Introduceti Orasul!" /><aui:option label="t" value="ttt;" /></aui:select> --%>
...
function LoadCities(judetSelectat){
	var citites = document.getElementById('hidCities').value;
	
	var selected = judetSelectat.value;
	
	console.log("judetSelectat= "+selected);
	
	console.log("cities = "+ citites);
	
	var citiesWithTagArray = citites.split(','); //RESPONSE + "^" + DISPLAY + "|" + TAG);
	
	var cititesFaraTag;
	
	for (var city : citiesWithTagArray){
		cititesFaraTag = city.split("|");
	}
	return true;
